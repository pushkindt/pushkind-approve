...
def SplitOrder(order_id):
    ...
    if form.validate_on_submit():
        ...
        for product_list in product_lists:
            ...
            categories = [
                product.get("categoryId", -1) for product in new_order.products
            ]
...
def DuplicateOrder(order_id):
    ...
    flash(
        f"Заявка успешно клонирована. Номер новой заявки {new_order.number}. "
...
def SaveQuantity(order_id):
    ...
    if form.validate_on_submit():
        ...
        if product["quantity"] != form.product_quantity.data:
            message = (
                f"{product['sku']} количество было {product['quantity']} "
        ...
        approvals = (
            ...
            .filter(
                OrderApproval.order_id == order_id, User.hub_id == current_user.hub_id
            )
            .all()
        )
...
def GetExcelReport1(order_id):
    ...
    starting_row = 11
    wb = load_workbook(
        filename=os.path.join("app", "static", "upload", "template.xlsx")
    )
    ws = wb.active
...
def GetExcelReport2(order_id):
    ...
    starting_row = 2
    wb = load_workbook(
        filename=os.path.join("app", "static", "upload", "template2.xlsx")
    )
    ws = wb.active
...
def Prepare1CReport(order, excel_date):
    ...
    if data_len > 0:
        ...
        starting_row = 3
        wb = load_workbook(
            filename=os.path.join("app", "static", "upload", "template1C.xlsx")
        )
        wb.iso_dates = True
        ws = wb["Заявка"]
        for merged_cell in ws.merged_cells.ranges:
        ...
        for k, i in enumerate(range(starting_row, starting_row + data_len)):
            ...
            ws.cell(i, 11).value = (
                order.income_statement.name
                if order.income_statement is not None
                else ""
            )
            ws.cell(i, 12).value = (
                order.cashflow_statement.name
            ...
            if "selectedOptions" in product:
                ...
                ws.cell(i, 23).value = ", ".join(
                    p["value"] for p in product["selectedOptions"][1:]
                )
            # Product Name
...
def GetExcelReport1C(order_id):
    ...
    if excel_send is False:
        return Response(
            ...
            mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={
                "Content-Disposition": f"attachment;filename=pushkind_{order.number}.xlsx"
            },
        )
...
def SaveApproval(order_id):
    ...
    if form.validate_on_submit():
        ...
        if form.product_id.data is None:
            ...
            position_disapprovals = position_disapprovals.filter(
                User.position_id == current_user.position_id
            ).all()
            ...
            OrderApproval.query.filter_by(
                order_id=order_id, user_id=current_user.id, product_id=None
            ).delete()
        ...
        if order.status != last_status:
            if order.status == OrderStatus.approved:
                ...
                if (
                    ...
                    and app_data.email_1C is not None
                    and app_data.notify_1C is True
                ):
...
@login_required
@role_required(
    [UserRoles.admin, UserRoles.initiative, UserRoles.validator, UserRoles.purchaser]
)
...
def SaveParameters(order_id):
    ...
    project = Project.query.filter_by(
        id=form.project.data, hub_id=current_user.hub_id
    ).first()
    ...
    if form.validate_on_submit() is True:
        new_site = Site.query.filter_by(
            id=form.site.data, project_id=form.project.data
        ).first()
        if new_site is not None and (
            order.site is None or order.site.id != new_site.id
        ):
