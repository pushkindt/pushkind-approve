...
class EcwidAPI():
    ...
    def GetStoreToken(self):
        ...
        if 'access_token' not in json:
            raise EcwidAPIException(
                'Не удалось получить токен доступа к магазину {}.'.format(self.id))
    ...
    def GetStoreEndpoint(self, endpoint, **kwargs):
        ...
        try:
            response = get(_REST_API_URL.format(
                store_id=self.id, endpoint=endpoint), params=params)
        except RequestException:
            raise EcwidAPIException('Ошибка обращения по API.')
        if response.status_code != 200:
            raise EcwidAPIException(
                self._GetErrorMessage(response.status_code))
        try:
    ...
    def DeleteStoreOrder(self, order_id):
        ...
        try:
            response = delete(_REST_API_URL.format(
                store_id=self.id, endpoint='orders/{}'.format(order_id)), params=params)
        except RequestException:
            raise EcwidAPIException('Ошибка обращения по API.')
        if response.status_code != 200:
            raise EcwidAPIException(
        ...
        if json.get('deleteCount', 0) == 0:
            raise EcwidAPIException(
                'Не удалось удалить заявку {}.'.format(order_id))
        return json
    ...
    def UpdateStoreOrder(self, order_id, order):
        ...
        params = {'token': self.token}
        try:
            response = put(_REST_API_URL.format(
                store_id=self.id, endpoint='orders/{}'.format(order_id)), params=params, json=order)
        except RequestException:
            raise EcwidAPIException('Ошибка обращения по API.')
        if response.status_code != 200:
            raise EcwidAPIException(
        ...
        if json.get('updateCount', 0) == 0:
            raise EcwidAPIException(
                'Не удалось обновить заявку {}.'.format(order_id))
        return json
    ...
    def OrderInvoice(self, order_id):
        ...
        try:
            response = get(_REST_API_URL.format(
                store_id=self.id, endpoint='orders/{}/invoice'.format(order_id)), params=params)
        except RequestException:
            raise EcwidAPIException('Ошибка обращения по API.')
        if response.status_code != 200:
            raise EcwidAPIException(
    ...
    def CreateStore(self, name, email, password, plan, **kwargs):
        ...
        payload = {'name': name, 'email': email, 'password': password,
                   'plan': plan, 'key': self.partners_key, **kwargs}
        params = {'register': 'y'}
        try:
            response = post(urljoin(_PARTNERS_API_URL, 'register'),
    ...
    def DeleteStore(self):
        ...
        if response.status_code == 403:
            raise EcwidAPIException(
                'Недостаточно прав на удаление поставщика {}.'.format(self.id))
        elif response.status_code != 200:
            raise EcwidAPIException('Неизвестная ошибка API.')
    ...
    def UpdateStoreProfile(self, template=None):
        ...
        try:
            response = put(_REST_API_URL.format(
                store_id=self.id, endpoint='profile'), json=template, params=params)
        except RequestException:
            raise EcwidAPIException('Ошибка обращения по API.')
        if response.status_code != 200:
            raise EcwidAPIException(
        ...
        if json.get('updateCount', 0) == 0:
            raise EcwidAPIException(
                'Не удалось обновить профиль поставщика {}.'.format(self.id))
        return json
    ...
    def SetStoreOrder(self, order):
        ...
        params = {'token': self.token}
        try:
            response = post(_REST_API_URL.format(
                store_id=self.id, endpoint='orders'), json=order, params=params)
        except RequestException:
            raise EcwidAPIException('Ошибка обращения по API.')
        if response.status_code != 200:
    ...
    def DeleteStoreProduct(self, product_id):
        ...
        params = {'token': self.token}
        response = delete(_REST_API_URL.format(
            store_id=self.id, endpoint='products/{}'.format(product_id)), params=params)
        if response.status_code != 200:
            raise Exception(self._GetErrorMessage(response.status_code))
        try:
            json = response.json()
        ...
        if json.get('deleteCount', 0) == 0:
            raise EcwidAPIException(
                'Не удалось удалить товар {}.'.format(product_id))
        return json
