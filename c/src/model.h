...
extern void FreeCategory(TCategory *cache);

extern TEcwid *GetStores(TDatabase *pDB, uint64_t hub_id, size_t *stores_count, uint64_t store_id);
extern char *GetStoreNameById(TDatabase *pDB, uint64_t store_id);
extern int StoreEcwidProfile(TDatabase *pDB, TEcwid store);
extern int UpdateEcwidProfile(TDatabase *pDB, TEcwid store);
extern int DeleteEcwidProfile(TDatabase *pDB, uint64_t store_id);

extern int StoreCategory(TDatabase *pDB, TCategory *cache);
extern TCategory *GetCategoryByChildId(TDatabase *pDB, uint64_t hub_id, uint64_t child_id);
